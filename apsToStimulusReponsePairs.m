function apsToStimulusReponsePairs(outputFile,spikeTimess,stimulusTimess,sequenceFile,blankStimulus,conditions,channelNames)
    ...
    responses = cell(nCells,1); % not a 5D matrix as that's likely to cause OOM errors
    
    for ii = 1:nCells
        responses{ii} = inf(size(stimuli,1),2,2,4);
    end
        ...
        for jj = 1:nStimuli
            ...
            for kk = 1:2
                ...
                for ll = 1:nCells
                    ...
                    responses{ll}((nStimuli/nWidths)*(ii-1)+ns(widthIndex),1,kk,widthIndex) = numel(spikesInTrial);
                    
                    if ~isempty(spikesInTrial > 0)
                        responses{ll}((nStimuli/nWidths)*(ii-1)+ns(widthIndex),2,kk,widthIndex) = spikesInTrial(1);
                    end
                end
            end
