...
nConditions = nContrasts*nPhases;
...
for kk = 1:nTrials
    ...
    for ii = 1:nContrasts
        for jj = 1:nPhases
            stims{sub2ind([nPhases nContrasts],jj,ii)} = squareWave(maxLuminances(ii),width*2,Inf,pi/2,stimTime,minLuminances(ii),phases(jj));
        end
    end
...
% !!! FILL IN SAVE FILE !!!
stimulate(getPixels,textureRect,[],markerRect,255,0,60,'D:\John B\Electrophysiology\Wild-Type Stim\JBWT0046\Stim 2.mat',false,false,false,cell(size(getPixels)));
