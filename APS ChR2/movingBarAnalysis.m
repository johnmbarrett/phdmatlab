function movingBarAnalysis(retina,detectionRecordings,useAllResponders,recomputeLatencies,replotRasters,fileSuffix,recomputeBayesianDecoderPerformance)
    if nargin < 7
    ...
    if isMCS
        ...
        if useAllResponders
            for ii = 1:2
                units{ii} = responsive{detectionRecordings(ii)}; %#ok<USENS>
            end
        else
...
%             embedded = binlessembedu(warped);
            ...
            allMutualInformation(hh,ii) = I.value;
        end
    end
    ...
    if recomputeBayesianDecoderPerformance
        ...
        for jj = 2:-1:1
            for kk = 1:2
                ...
                [fc,~,pvu] = simpleBayesianDecoder(x,y,[],subUnits,10,'kde',0,pathTime(kk)+0.05*kk);
