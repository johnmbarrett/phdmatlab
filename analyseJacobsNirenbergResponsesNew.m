function analyseJacobsNirenbergResponses(recording,varargin)
    ...
    opts = getopt('latencytype=''firstspike'' significance=''rf'' usesavedposteriors=false ifrfit=''gauss'' ifrparam=1 latencyparam=50 posteriorsonly=false seed=NaN partition=''halfnhalf'' reps=50',varargin{:});
    ...
    %     responsiveChannels = [13 2; 17 1; 35 2; 36 1; 37 1; 37 2; 48 2; 58 1; 62 1; 71 2; 73 1];
        ...
        for ii = 1:nResponses
            ...
            channel = cells(cellIndex,1:2); %#ok<FDEPR>
            cluster = cells(cellIndex,3); %#ok<FDEPR>
        ...
        [phases,phaseIndices] = sort(phases); %#ok<NASGU>
            ...
            if strcmpi(opts.partition,'halfnhalf')
                nReps = 1;
            ...
            elseif strcmpi(opts.partition,'allbutone')
                nReps = opt.reps;
                randomIndices = randi(nStimuli,nReps,1);
                trainIndices = sortedStimulusIndices;
                testIndices = sortedStimulusIndices(randomIndices);
            end
...
%         if false
        ...
        %     sortedTimings = squeeze(stimulusTimings(widthIndices,shiftIndices,1));
            ...
            psths = {zeros(numel(binss{1}),8,4,nResponses,nReps) zeros(numel(binss{2}),8,4,nResponses,nReps)};
        %     latencyHists = {zeros(numel(binss{1})+1,8,4,nResponses) zeros(numel(binss{2})+1,8,4,nResponses)};
            isihs = cell(8,4,nResponses,2,nReps);
            ...
            allCounts = zeros(numel(trainIndices),2,nResponses,nReps);
            allLatencies = zeros(numel(trainIndices),2,nResponses,nReps);
            allPhases = zeros(numel(trainIndices),nReps);
            allWidths = zeros(numel(trainIndices),nReps);
            ...
            for ii = 1:nTrainTrials
                ...
                repIndices = setdiff(1:nReps,find(ismember(testIndices,trainIndices(ii))));
                
                ns(phaseIndex,widthIndex,repIndices) = ns(phaseIndex,widthIndex,repIndices) + 1;
                allPhases(ii,repIndices) = phaseIndex;
                allWidths(ii,repIndices) = widthIndex;
        ...
        %                 tic;
                        ...
                        allCounts(ii,jj+1,kk,repIndices) = numel(cellSpikeIndices);

                        if isempty(cellSpikeIndices)
                            allLatencies(ii,jj+1,kk,repIndices) = Inf;
        ...
        %                         latencyHists{jj+1}(end,phaseIndex,widthIndex,kk) + 1;
                        ...
                        if strcmp(opts.latencytype,'burst')
                            if numel(spikeTimes) == 1
                                allLatencies(ii,jj+1,kk,repIndices) = spikeTimes(1)-interval(1);
                            else
                                ...
                                if isempty(burstStarts)
                                    ...
                                    allLatencies(ii,jj+1,kk,repIndices) = spikeTimes(burstStarts(1))-interval(1);
                                end
                            end
                        elseif strcmp(opts.latencytype,'peak')
                            ...
                            allLatencies(ii,jj+1,kk,repIndices) = index*binWidth;
                        else
                            allLatencies(ii,jj+1,kk,repIndices) = spikeTimes(1)-interval(1);
                        end
        ...
        %                     latencyHists{jj+1}(index,phaseIndex,widthIndex,kk) + 1;
                        ...
                        psths{jj+1}(spikeBins,phaseIndex,widthIndex,kk,repIndices) = ...
                            psths{jj+1}(spikeBins,phaseIndex,widthIndex,kk,repIndices) + 1;
        ...
        %                 if isempty(nextSpike)
                        ...
                        for mm = 1:numel(repIndices)
                            repIndex = repIndices(mm);
                            
                            isih = isihs{phaseIndex,widthIndex,kk,jj+1,repIndex};
                            ...
                            isihs{phaseIndex,widthIndex,kk,jj+1,repIndex} = isih;
                        end
        ...
        %                 toc;
            ...
            phasePDFs = zeros(8,4,nReps);

            maxCount = max(max(max(allCounts)));
            countPDFs = zeros(maxCount+1,2,nResponses,nReps);

            for ii = 1:nResponses
                for jj = 1:2
                    for kk = 1:nReps
                        counts = allCounts(:,jj,ii,kk);
                        countPDFs(:,jj,ii,kk) = hist(counts,0:maxCount)/numel(counts);
                    end
                end
            end

            countPhaseConditionalPDFs = zeros(maxCount+1,8,nResponses,2,4,nReps);

            ratess = {zeros(size(psths{1})) zeros(size(psths{2}))};
            isirs = cell(size(isihs));

            kdePoints = 2^7;
            infLatencyPhaseConditionalPDFs = zeros(8,4,2,nResponses,nReps);
            latencyPhaseConditionalPDFSupports = zeros(kdePoints,8,4,2,nResponses,nReps);
            latencyPhaseConditionalPDFs = zeros(kdePoints,8,4,2,nResponses,nReps);

            for ii = 1:4
                for jj = 1:8
                    for mm = 1:nReps
                        indices = allPhases(:,mm) == jj & allWidths(:,mm) == ii;
                        n = sum(indices);
                        phasePDFs(jj,ii) = n;
                        ...
                        for kk = 1:2
                            ...
                            for ll = 1:nResponses
                                ...
                                tic;
                                counts = allCounts(indices,kk,ll,mm);
                                countPhaseConditionalPDFs(:,jj,ll,kk,ii,mm) = hist(counts,0:maxCount)/numel(counts);

                                latencies = allLatencies(indices,kk,ll,mm);
                                finiteLatencies = latencies(isfinite(latencies));

                                infLatencyPhaseConditionalPDFs(jj,ii,kk,ll,mm) = 1-numel(finiteLatencies)/numel(latencies);

                                if ~isempty(finiteLatencies)
                                    [~,pdf,ts] = kde(latencies,kdePoints,t0,t1);
                                    latencyPhaseConditionalPDFSupports(:,jj,ii,kk,ll,mm) = ts;
                                    latencyPhaseConditionalPDFs(:,jj,ii,kk,ll,mm) = pdf/sum(pdf);
                                end

                                rate = (psth(:,jj,ii,ll,mm)/n)/binWidth;
                                maxRate = max(rate);
                                ...
                                elseif strcmpi(opts.ifrfit,'kde')
                                    tic;
                                    PSTH = psth(:,jj,ii,ll,mm);
                                    data = zeros(sum(PSTH),1);
                                    index = [0; cumsum(PSTH)];

                                    for nn = 1:size(PSTH,1)
                                        data(index(nn)+1:index(nn+1)) = bins(nn)*ones(PSTH(nn),1);
                                    end
                                ...
                                ratess{kk}(:,jj,ii,ll,mm) = rate;

                                isir = (isihs{jj,ii,ll,kk,mm}/n)/binWidth;
            ...
            phasePDFs = phasePDFs./repmat(sum(phasePDFs),[8 1 nReps]);
%         else
%             load('lalala');
%         end
...
%         end
        ...
        for mm = 1:numel(nNeurons)
            ...
            for jj = 1:repeats
                ...
                allCellsPosterior = squeeze(sum(safelog(posteriors(:,ii,indices,:,:)),3));
...
%     save([fileDir '\' recording.dataFile '_bayes.mat'],'countPerformance','timingPerformance','correlationPerformance');
    save([fileDir '\' filename '_bayes_' opts.ifrfit '_' num2str(opts.ifrparam) '_' opts.latencytype '_' num2str(opts.latencyparam) '.mat'],'performances','-append');
