function plotRFs(recording,varargin)
    fileDir = getAnalysisOutputDir(recording);
    
    figure;
    hold on;
    
    ellipse = @(a,b,x0,y0) @(sign) @(x) sign*sqrt(b.^2.*(1-((x-x0).^2./a.^2))) + y0;
    ...
    for ii = 1:numel(srrfs)
        ...
        f = ellipse(2*sdXOn,2*sdYOn,centreXOn,centreYOn);
        xlim = centreXOn+2*sdXOn*[-1 1];
        fplot(f(1),xlim);
        fplot(f(-1),xlim);
    end
end
