...
function [rate, bursttime, burstend, burstdur, burstsize, testelecs, isbad, meanrate, wlen, wskip] = findAllBurstsAPS2(files)
%find all the bursts in the given spike files.
%if no outfile is given, it will not be saved, but the data is return to
%the calling function. This function was rewritten to return the data so
...
for x=1:64,
  for y=1:64,
    chname=['Ch',num2str(x),'_',num2str(y)];
    if exist(chname)
      [i,j] = find(eval(chname));
      if length(i)>minNSpikes & length(i)
	epos(c,1)=x;
	epos(c,2)=y;
	spikes.times{c} = i/freq;

	nspikes(c) = length(spikes.times{c});
	maxt(c) = max(spikes.times{c});
	elecs = [elecs c];
	c=c+1;
      end
    end
...
for enum = ee,
  ...
  wlen(enum) = spiketrate_window;
  wskip(enum) = wlen(enum)/scaling;
  ...
  addbit = scaling-1;
  number_of_windows = max(times_in_windows)-scaling;
  scount = zeros(1, max(times_in_windows)-scaling);
  ...
  [n v] = hist(scount,200);
  tmp = find((fliplr(cumsum(fliplr(n/sum(n)))))<scthres);
...
outfile = [name,'_bursts',ext]
%outfile = [path,'/',name,'_bursts',ext]
