function calculateSpikeTriggeredAverage(recording,varargin)
    ...
    for ii = 1:4
        for jj = 1:nCells
            stass{jj,ii} = zeros(textureWidth,textureHeight,0);
        end
    ...
    for ii = 1:length(vbls)
        ...
        for jj = 1:2
            ...
            for kk = 1:length(spikes)
                stasAllSpikes = stass{kk,2*jj-1};
                stasFirstSpikes = stass{kk,2*jj};
                spike = spikes(kk);
    ...
    muSTA = cell(size(stass));
    sigmaSTA = cell(size(stass));
    zSTA = cell(size(stass));
