function [p, h, stats] = matrixranksum(x,y,varargin)
...
%    if nx+ny<=10 || isequal(method,'oldexact')
      ...
      for ii = 1:size(ranks,2)
          allpos = nchoosek(ranks(:,ii),ns);
          sumranks = sum(allpos,2);
          np = length(sumranks);
          plo = sum(sumranks<=w(ii))/np;
          phi = sum(sumranks>=w(ii))/np;
          p(ii) = min(plo,phi);
      end
%    else
...
%    end
   ...
   p = min(2*p, ones(size(p)));           % 2-sided, p>1 means the middle is double-counted
